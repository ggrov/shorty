...
namespace shorty
{
    internal class Shorty
    {
        ...
        //Lemma Calls
        public readonly Dictionary<Statement, List<UpdateStmt>> lemmaCalls = new Dictionary<Statement, List<UpdateStmt>>(); //The type of lemma calls we want to remove are inside UpdateStatements
            ...
            foreach (Statement stmt in lemmaCalls.Keys) {
                if (stmt is BlockStmt) {
                    ...
                    for (int i = lemmaCalls[block].Count - 1; i >= 0; i--) {
                        ...
                        if (!IsProgramValid()) { //for some reason this passes when it shouldnt
...
//                    }
        ...
        private void TryToRemoveAssert(AssertStmt assert, BlockStmt block,
            Dictionary<AssertStmt, Statement> removedParentsDictionary, List<AssertStmt> removedCaseAsserts)
        {
        ...
        {
            if (!IsProgramValid()) {
                Console.WriteLine("Program " + program.FullName +
                                  " cannot verify -> cannot remove asserts until it is fixed");
                return null;
            }
                            ...
                            if (!assertsToKeep.Contains(assert)) {
                                ...
                                if (mode == Mode.All) {
                                    if (!removedCaseStmts.ContainsKey(assert) &&
                                        !removedParentsDictionary.ContainsKey(assert))
            ...
            if (mode == Mode.All) {
                ...
                if (!IsProgramValid()) {
                    ...
                    else{Console.WriteLine("Not fixed");}
                }
                else {
        ...
        {
            if (mode == Mode.All) { // This is not working
