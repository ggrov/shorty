...
namespace shorty
{
    internal class Shorty
    {
        ...
        //Asserts
        private readonly Dictionary<Statement, List<AssertStmt>> _asserts = new Dictionary<Statement, List<AssertStmt>>();
        private readonly Dictionary<Statement, List<AssertStmt>> _andAsserts = new Dictionary<Statement, List<AssertStmt>>();
        private readonly Dictionary<Method, List<Tuple<AssertStmt, Statement>>> _assertsParents = new Dictionary<Method, List<Tuple<AssertStmt, Statement>>>();
        ...
        public void PrintAsserts()
        {
            ...
            foreach (var block in _asserts.Keys) {
                Console.WriteLine("Block " + ++i + ", Line " + block.Tok.line);
                foreach (var assert in _asserts[block]) {
...
//            { Console.WriteLine("Member not method"); }
        ...
        private void CheckStatement(Statement statement, Statement parent, Method method)
        {
            ...
            if (statement is AssertStmt) {
                ...
                if (!_assertsParents.ContainsKey(method)) {
                    _assertsParents.Add(method, new List<Tuple<AssertStmt, Statement>>());
                }
                _assertsParents[method].Add(new Tuple<AssertStmt, Statement>(assert, parent));
                if (_asserts.ContainsKey(parent)) {
                    if (!_asserts[parent].Contains(statement)) {
                        _asserts[parent].Add((AssertStmt) statement);
                    }
                }
                else {
                    _asserts.Add(parent, new List<AssertStmt> {(AssertStmt) statement});
                }
        ...
        void GetDecreasesLoop(LoopStmt loop, Method method)
        {
            if (loop.Decreases.Expressions.Count > 0) {
                if (!_decreases.ContainsKey(method)) {
                    _decreases.Add(method, new List<LoopStmt>());
                }
                _decreases[method].Add(loop);
            }
        }
        ...
        {
            if (!_decreases.ContainsKey(method) && method.Decreases.Expressions.Count > 0) {
                _decreases.Add(method, new List<LoopStmt>());
            }
...
//                    }
                ...
                foreach (LoopStmt loop in _decreases[method]) {
        ...
        public Dictionary<Method, List<List<AssertStmt>>> TestDifferentRemovals()
        {
            ...
            foreach (Method method in _assertsParents.Keys) {
        ...
        {
            if (index == _assertsParents[method].Count) {
                solutions.Add(new List<AssertStmt>(currentSolution));
                return;
            }

            var parent = _assertsParents[method][index].Item2;
            var assert = _assertsParents[method][index].Item1;
        ...
        public List<Tuple<AssertStmt, AssertStmt>> GetSimplifiedAsserts()
        {
            ...
            foreach (Statement stmnt in _andAsserts.Keys) {
                ...
                foreach (AssertStmt assert in _asserts[stmnt]) {
        ...
        public List<AssertStmt> FindUnnecessaryAsserts()
        {
            ...
            foreach (Statement stmnt in _asserts.Keys) {
