...
//            using (TextWriter writer = File.CreateText("C:\\users\\Duncan\\Documents\\results.txt")) {
        ...
        private static bool TestRemovalOrderingOnProgram(Program program, TextWriter writer)
        {
            ...
            try {
                ...
                var removalOrderTesterAssert = new RemovalOrderTester<Statement>(daryController.AllRemovableTypes.GetAssertDictionary(), program);
                var assertSolutions = removalOrderTesterAssert.TestDifferentRemovals();

                var removalOrderTesterInvar = new RemovalOrderTester<MaybeFreeExpression>(daryController.AllRemovableTypes.GetInvariantDictionary(), program);
